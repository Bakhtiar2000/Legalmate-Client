...
const AttorneyDiv = ({ attorney }) => {
  ...
  const presentEmployment = experience.filter(
    (exp) => exp?.end_year === "Present"
  );
    ...
    <div className="relative group rounded-lg p-3 md:p-5 max-w-5xl bg-lightDark border border-primary mb-5 shadow-lg hover:shadow-white/40 duration-300 mx-auto md:flex gap-8">
      <div className="flex gap-5">
        ...
        <div className="md:hidden block">
          <div>
            <Link
              to={`/attorney_details/${_id}`}
              className="hover:text-primary font-semibold text-xl md:text-2xl cursor-pointer duration-300 w-fit inline"
            >
              {" "}
              {name}{" "}
            </Link>
            {practiceArea && (
              <span className="block mb-1 text-sm sm:text-base duration-300">
                ({practiceArea} Law Specialist)
              </span>
            )}
          </div>
          <p className="text-primary">
            {presentEmployment[0]?.position} at {presentEmployment[0]?.company}
          </p>
      ...
      <div>
        ...
        <div className="hidden md:block">
          <div>
            <Link
              to={`/attorney_details/${_id}`}
              className="hover:text-primary font-semibold text-2xl cursor-pointer duration-300 w-fit inline"
            >
              {" "}
              {name}{" "}
            </Link>
        ...
        {license && license.length !== 0 && (
          <p className="text-orange-500">
            Licensed for {currentYear - license.acquired_year} years
          </p>
        )}
        <p className="line-clamp-3 text-sm md:text-base duration-300">
          {about}
        </p>
      </div>
      ...
      <Link
        to={`/attorney_details/${_id}`}
        className="bg-primary/50 text-white group-hover:inline-block hidden p-3 rounded-md absolute top-3 right-3 md:-right-16 group-hover:right-3 duration-300 hover:bg-primary shadow-xl shadow-purple/20 hover:shadow-white/20"
      >
