...
const CaseFilter = () => {
  ...
  const approvedCases = allCasesData.filter(
  ...
  useEffect(() => {
    ...
    let filter = approvedCases.filter(
      (data) =>
        (!searchName ||
          (data.writer && data.writer.toLowerCase().includes(searchName))) &&
        (!searchLocation ||
          (data.location &&
            data.location.toLowerCase().includes(searchLocation))) &&
        (!searchPracticeArea ||
          (data.practice_area &&
            data.practice_area.toLowerCase().includes(searchPracticeArea)))
    );
  ...
  return (
    <div className="container py-20">
      ...
      <div className="max-w-6xl mx-auto duration-300 mb-10">
        ...
        >
          {/* name */}
          <div className="col-span-3 flex items-center border-b md:border-none border-primary">
          ...
          {/* practice area */}
          <div className="col-span-3 border-b md:border-b-0 md:border-s border-primary md:border-e flex items-center">
            ...
            >
              ...
              {Array.from(
                new Set(practiceAreasData.map((area) => area.name))
              ).map((name, index) => (
      ...
      {filteredData.length !== 0 ? (
        filteredData.map((singleCase) => (
          <CaseDiv key={singleCase._id} singleCase={singleCase}></CaseDiv>
        ))
      ) : (
        <p className="py-4 px-6 sm:text-lg max-w-5xl mx-auto text-center bg-lightDark rounded-lg">
          ☹ No case Found!
        </p>
      )}
    </div>
