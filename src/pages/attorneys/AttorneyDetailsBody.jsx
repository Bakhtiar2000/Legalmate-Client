...
const AttorneyDetailsBody = ({ singleAttorney }) => {
  ...
  useEffect(() => {
    if (currentUser?.role === "client") {
      const senderId = clientsData?.filter(
        (client) => client.email === currentUser?.email
      );
      setSenderId(senderId[0]?._id);
    } else if (currentUser?.role === "attorney") {
      const senderId = attorneysData?.filter(
        (attorney) => attorney.email === currentUser?.email
      );
      setSenderId(senderId[0]?._id);
    }
  ...
  useEffect(() => {
    paymentData?.map((pay) => {
      if (pay.target_role === "attorney") {
        const paymentStatus =
          pay.target_id === _id &&
          pay.target_email === email &&
          pay.sender_email === currentUser?.email &&
          pay.sender_name === currentUser?.name &&
          pay.isPaid === true;
        if (paymentStatus) {
        ...
        <div className="flex flex-col gap-8">
          ...
          <div className="md:flex justify-between gap-8 mt-5 md:mt-0">
            ...
            <div>
              ...
              <p className="lg:text-xl md:mt-2">
                {practiceArea} lawyer from {location}
              </p>
              {presentEmployment && (
                <p className="lg:text-xl md:mt-2">
                  {presentEmployment[0]?.position} at{" "}
                  {presentEmployment[0]?.company}
                </p>
              )}
            ...
            <div className="bg-lightDark/50 rounded-lg p-3 md:ml-5 border border-dashed border-white h-fit w-fit">
              <p className="text-2xl border-b pb-3 border-dark mb-5">
          ...
          {about && (
            <div className="bg-primary/20 px-5 py-3 rounded-lg max-w-3xl">
              <div>
                <p className={`line-clamp-${showFullText ? "none" : "3"}`}>
                  {about}
                </p>
                {!showFullText ? (
                  <button
          ...
          <div className="flex justify-center gap-5">
            ...
            <button
              onClick={createChat}
              disabled={true}
              className="lg:text-xl text-center flex-1 disabled:cursor-not-allowed disabled:opacity-50"
            >
            ...
            {paymentSuccess ? (
              <button
                onClick={createChat}
                className="lg:text-xl text-center flex-1"
              >
                <div className="mt-auto w-full bg-green-600 hover:bg-green-600/60 duration-300 rounded-lg px-2 py-3 cursor-pointer text-center">
                  Message
                </div>
